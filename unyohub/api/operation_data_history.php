...
if (isset($_POST["formation_name"])) {
    ...
    $sql_r = $db_obj->query("SELECT `operation_date`, `operation_number`, `formations` FROM `unyohub_assigned_formation_caches` WHERE `operation_number` = '".$db_obj->escapeString($_POST["operation_number"])."' AND `operation_date` <= '".$end_date."' AND `operation_date` >= '".$start_date."' AND `formations` IS NOT NULL ORDER BY `operation_date` ASC, `assign_order` DESC");
} else {
    print "ERROR: 送信値が不正です";
    exit;
}
