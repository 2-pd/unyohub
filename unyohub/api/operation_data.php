...
$operations_r = $db_obj->query("SELECT `operation_number`, `formations`, `posts_count`, `variant_exists`, `comment_exists`, `from_beginner`, `updated_datetime` FROM `unyohub_data_caches` WHERE `operation_date` = '".$db_obj->escapeString($_POST["date"])."' AND `updated_datetime` > '".date("Y-m-d H:i:s", intval($_POST["last_modified_timestamp"]))."' ORDER BY `updated_datetime` DESC");
...
while ($operation = $operations_r->fetchArray(SQLITE3_ASSOC)) {
    ...
    if (!is_null($operation["formations"])) {
        $operation_data[$operation["operation_number"]] = array("formations" => $operation["formations"], "posts_count" => $operation["posts_count"], "variant_exists" => boolval($operation["variant_exists"]), "comment_exists" => boolval($operation["comment_exists"]), "from_beginner" => boolval($operation["from_beginner"]));
    } else {
