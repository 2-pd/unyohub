...
if (empty($wakarana->get_permission("instance_administrator"))) {
    $wakarana->create_permission("instance_administrator", "インスタンス管理者");
}

if (empty($wakarana->get_permission("control_panel_user"))) {
    $wakarana->create_permission("control_panel_user", "コントロールパネル使用者");
}

if (empty($wakarana->get_permission("management_member"))) {
    $wakarana->create_permission("management_member", "運営メンバー");
}

if (empty($wakarana->get_permission("railroads"))) {
    $permission_railroads = $wakarana->create_permission("railroads", "路線系統データの編集・管理");
