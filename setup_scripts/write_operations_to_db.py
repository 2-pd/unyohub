...
for operation_group in operation_groups:
    operations = operation_data[operation_group]["operations"].keys()
    for operation in operations:
        cur.execute("INSERT INTO `unyohub_operations` (`operation_table`, `operation_number`, `starting_location`, `terminal_location`) VALUES (:operation_table, :operation_number, :starting_location, :terminal_location)", {"operation_table" : operation_table, "operation_number" : operation, "starting_location" : operation_data[operation_group]["operations"][operation]["starting_location"], "terminal_location" : operation_data[operation_group]["operations"][operation]["terminal_location"]})
        
        trains = operation_data[operation_group]["operations"][operation]["trains"].keys()
        for train in trains:
            if train[0:1] == "_" or "__" in train:
                continue
            
            for train_data in operation_data[operation_group]["operations"][operation]["trains"][train]:
