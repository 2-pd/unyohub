    ...
    if formation_data[cnt][0].startswith("# "):
        ...
        for cnt_2 in range(1, len(formation_data[cnt])):
            if formation_data[cnt][cnt_2] != "":
                ...
                cur.execute("INSERT INTO `unyohub_cars`(`formation_name`, `car_number`, `manufacturer`, `constructed`, `description`) VALUES (:formation_name, :car_number, :manufacturer, :constructed, '') ON CONFLICT(`formation_name`, `car_number`) DO UPDATE SET `formation_name` = :formation_name_2, `car_number` = :car_number_2, `manufacturer` = :manufacturer_2, `constructed` = :constructed_2", {"formation_name" : formation_name, "car_number" : car_number, "manufacturer" : formation_data[cnt + 2][cnt_2], "constructed" : formation_data[cnt + 3][cnt_2], "formation_name_2" : formation_name, "car_number_2" : car_number, "manufacturer_2" : formation_data[cnt + 2][cnt_2], "constructed_2" : formation_data[cnt + 3][cnt_2]})
