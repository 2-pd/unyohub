...
※オプションとして「--help」または「-h」を指定するとヘルプが表示される
...
### 引数
**第1引数** : ユーザー識別名(省略時はユーザーの総数を表示)


## unyohub create-user
ユーザーを新規登録する  
処理本体は**create_user.php**
...
## unyohub modify-user-info
ユーザーの情報を変更する  
処理本体は**modify_user_info.php**

### 引数
**第1引数** : ユーザー識別名

### オプション
**--user-name "ユーザー表示名"** : ユーザー表示名を変更する場合は指定  
**--email-address "メールアドレス"** : メールアドレスを変更する場合は指定  
**--website-url "webサイトのURL"** : webサイトのURLを変更する場合は指定  
**--reset-password** : パスワードを再設定する場合は指定(新しいパスワードはランダムな文字列が自動設定される)  
**--time-out 日数** : タイムアウトを設定する場合は指定(日数を0とした場合はタイムアウトが解除される)  
**--enable** : アカウントの凍結を解除する場合は指定  
**--disable** : アカウントを凍結する場合は指定
