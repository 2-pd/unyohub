...
[  
    { 各お知らせの情報(有効期限が切れたものはサーバ側で除外される)  
        ...
        "show_on_post_screen" : 投稿画面に表示するか否か(BOOL値),  
        "content" : お知らせの本文,  
...
**$_POST["year_month"]** : YYYY-MM形式の年月(省略時は現在日から過去30日間の運用履歴を取得する)
...
{  
    "YYYY-MM-DD形式の日付" : [  
        {  
            ...
            "formations" : 編成名(前位側・奇数向きから順に各編成を「+」で区切った文字列。運休の場合は空文字列),  
            "relieved_formations" : 差し替え前の編成を充当順に配列で(差し替えがなければ省略),  
            "previous_day_operation_data" : [ この運用に充当された編成の前日の運用情報(編成名が指定された場合は省略)  
                {  
                    "operation_number" : 運用番号,  
                    "formations" : 編成名(前位側・奇数向きから順に各編成を「+」で区切った文字列。差し替えがあった場合は差し替え完了後のもの)  
                }...  
            ],  
            "former_operation_data" : [ この運用に充当された編成の当日出庫前の運用情報(編成名が指定された場合は省略)  
                {  
                    "operation_number" : 運用番号,  
                    "formations" : 編成名(前位側・奇数向きから順に各編成を「+」で区切った文字列。差し替えがあった場合は差し替え完了後のもの)  
                }...  
            ],  
            "latter_operation_data" : [ この運用に充当された編成の当日入庫後の運用情報(編成名が指定された場合は省略)  
                {  
                    "operation_number" : 運用番号,  
                    "formations" : 編成名(前位側・奇数向きから順に各編成を「+」で区切った文字列。差し替えがあった場合は差し替え前のもの)  
                }...  
            ],  
            "next_day_operation_data" : [ この運用に充当された編成の翌日の運用情報(編成名が指定された場合は省略)  
                {  
                    "operation_number" : 運用番号,  
                    "formations" : 編成名(前位側・奇数向きから順に各編成を「+」で区切った文字列。差し替えがあった場合は差し替え前のもの)  
                }...  
...
**正常時** :  
{  
    "編成名" : { 最終運行情報(一度も運行した記録がない編成では編成名のキーそのものが存在しない)  
...
{  
    ...
    "user_timed_out_logs" : [ ユーザーIDのタイムアウト指定履歴(直近5件を新しい順に。ユーザーIDが空の場合はNULL)  
        {  
            ...
            "timed_out_reason" : タイムアウトの理由(ない場合はnull),  
