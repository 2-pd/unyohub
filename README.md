...
本アプリケーションでは、unyohubコマンドによるバッチ処理を定期実行することで一部機能が利用可能となります。  
利用可能とするためにバッチ処理の定期実行が必要な機能と、それに対応するバッチ処理の実行設定は以下の通りです。  
**※ unyohubコマンドのパスはサーバ上での実際の絶対パスに読み替えてください。**

- **各運用の運用履歴画面での前日・翌日及び当日中の運用遷移表示機能**  
    この機能を有効化する場合は、crontabにて、毎日午前4時前後に前日分の運用遷移データのキャッシュを生成するコマンドが実行されるよう設定してください。例えば、毎日午前3時51分に前日運用遷移データのキャッシュを生成するには以下のように設定します。  
    `51 3 * * * /var/www/unyohub/commands/unyohub cache-transitions -s`
